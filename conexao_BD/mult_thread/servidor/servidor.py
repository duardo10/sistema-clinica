...
try:
    ...
    class BancoDeDados:
        ...
        def buscCons(self,cpf):
            try:
                ...
                if (pessoa != None):              
                    ...
                    cpf_recepcionista = cursor.fetchone()[0]
                    print('saiu daqui')
                    enviar = f'1,{nome},{telefone},{dt_nasc},{medico},{crm},{tipo_consulta},{cpf_medico},{cpf_recepcionista}'
                    conexao_servidor.send(enviar.encode())
                else:
                    enviar = '0'
                    conexao_servidor.send(enviar.encode())
        ...
        def listaPacientes(self):
            try:
                cursor.execute("SELECT cpf, nome FROM lista_pacientes WHERE medico_cpf = %s",(login_atual,))
